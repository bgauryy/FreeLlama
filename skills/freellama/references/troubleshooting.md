...
**Not a failure.** Admission is a cost-unit budget (embedding 1, chat 2, vision 4, 8 total) and a
task that cannot get a slot within 120s is refused, naming the cost and the budget. **Fix:** retry,
or lower your fan-out. `GET /_freellama/v1/health` reports `slots_available` before you submit, and
every success reports `admission.queue_wait_ms` so throttling is visible before it becomes a 503.
Raise the ceiling with `npx freellama serve --max-concurrent-tasks <n>`, but read
...
automatically. `scripts/check.sh` reports whether you are actually going through it.

## A request that used to fail fast now times out instead

**Cause:** retries without a bounded per-attempt timeout compound. One slow-but-eventually-failing
...
Next: changing retry/timeout behaviour rather than diagnosing it → `references/reliability.md`.
