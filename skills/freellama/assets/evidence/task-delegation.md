...
`run_task {preview:true}` / `doctor` / `models` only make *selection* decisions — they never do work.
...
- **`delegate_research`** hands a question to a local model wired to a shell (or `octocode`) agent
  ...
  of a heavier subprocess spawn. Its model turns are managed `coding` tasks, so placement intent,
  independent CPU/GPU admission, and physical receipts are preserved. Reach for it specifically
  when you need the citable trail.
