...
// CLI side: the first string of each 3-tuple in `print_tool_map`'s `rows` array. Each tuple opens
// with `(` at 8 spaces of indent, and the tool name is the next line at 12 spaces.
const rows = readFileSync(mainRs, "utf8").match(/fn print_tool_map[\s\S]*?\n\}/);
assert.ok(rows, "could not locate print_tool_map in packages/cli/src/main.rs");
const advertised = [...rows[0].matchAll(/^ {8}\(\n {12}"([a-z_]+)",/gm)].map((m) => m[1]).sort();

assert.ok(registered.length >= 8, `expected >=8 registered MCP tools, found ${registered.length}`);
assert.deepEqual(
  advertised,
  registered,
  `CLI tool map (packages/cli/src/main.rs::print_tool_map) has drifted from the MCP server.\n` +
    `  registered in index.ts : ${registered.join(", ")}\n` +
    `  advertised by the CLI  : ${advertised.join(", ")}\n` +
    `Update print_tool_map so the two match.`,
);

// The CLI header states the count in prose ("FreeLlama exposes N MCP tools"); keep it honest too.
const header = readFileSync(mainRs, "utf8").match(/exposes (\d+) MCP tools/);
assert.ok(header, "print_tool_map no longer states the tool count");
assert.equal(
  Number(header[1]),
  registered.length,
  `CLI says "${header[1]} MCP tools" but ${registered.length} are registered.`,
);

console.log(`tool-map-drift: OK — ${registered.length} tools match across MCP server and CLI`);
