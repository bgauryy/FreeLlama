...
pub fn qualify_from_aggregate(
    ...
    for entry in &aggregate.models {
        ...
        let matching = installed
            ...
            .filter(|model| {
                ...
                entry.id == model_slug || entry.id.starts_with(&format!("{model_slug}-"))
            })
            .collect::<Vec<_>>();
        let Some(longest) = matching.iter().map(|model| slug(model).len()).max() else {
            continue;
        };
        let finalists = matching
            .into_iter()
            .filter(|model| slug(model).len() == longest)
            .collect::<Vec<_>>();
        ensure!(
            finalists.len() == 1,
            "aggregate id `{}` ambiguously matches installed models {}; their lossy slugs are \
             indistinguishable, so refusing rather than crediting evidence to the wrong model",
            entry.id,
            finalists
                .iter()
    ...
    qualified.retain(|entry| seen.insert(entry.model.clone()));
